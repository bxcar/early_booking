...
jQuery(function ($) {
    ...
    /*$('#load-top-content').load('/js/content/?content_type=top',function(){
     dReady.init();
     });
     $('#load-bottom-content-1').load('/js/content/?content_type=bottom_1',function(){dReady.init();});
     $('#load-bottom-content-2').load('/js/content/?content_type=bottom_2',function(){dReady.init();});
     dReady.init();*/
});
...
var dReady = {
    ...
    init: function () {
        ...
        $('img[vspace]').each(function () {
            $(this).css({
                marginTop: $(this).attr('vspace') + 'px',
                marginBottom: $(this).attr('vspace') + 'px'
            }).removeAttr('vspace');
        });
        $('img[hspace]').each(function () {
            $(this).css({
                marginLeft: $(this).attr('hspace') + 'px',
                marginRight: $(this).attr('hspace') + 'px'
            }).removeAttr('hspace');
        });

        if (!this.isFancyBox && $.fn.fancybox) {
            this.isFancyBox = true;
            $('.fancybox').fancybox({
                prevEffect: 'fade',
                nextEffect: 'fade'
            });
        ...
        $('a[href="#orderform"]').click(function () {
            popup.show('#main-form');
            return false;
        })
    }
}
...
var siteDocReady = {
    ...
    datepickerInit: function () {
        if (isIphone) {
            $('input.site-date').each(function () {
                $(this).attr('type', 'date').css('line-height', '40px');
            });
            return false;
        }

        $('input.site-date:visible').datepicker({
            showAnim: '',
            minDate: nowDate,
            beforeShow: function (input) {
                $(input).addClass('site-date-active');
            },
            onClose: function (a, b) {
                $('#' + b.id).removeClass('site-date-active');
            }
        }).on('focus', function () {
            var input = $(this);
            input.blur();
    ...
    spinnerInit: function () {
        $('.spinner-item .control').each(function () {
            ...
            if (!c.data('isInit')) {
                ...
                if (input.size() == 0) var input = $('<input type="hidden" name="' + c.data('name') + '" value="' + val + '" />').appendTo(c);
                text.text(val);
    ...
    floatMenuInit: function () {
        $('.float-menu-container a').click(function () {
            var a = $(this)
            var c = a.closest('.float-menu-container');
            if (a.hasClass('active') || c.data('isAnimate')) return false;
            var active = $('a.active', c);
            if (active.size() == 0) return false;
            a.addClass('animated');
            var left = a.position().left, width = a.outerWidth();
            var bg = $('<div />').addClass('tmp').css({
                left: active.position().left + 'px',
                width: active.outerWidth() + 'px'
            });
            c.append(bg).data('isAnimate', true);
            active.removeClass('active');
            bg.animate({left: left, width: width}, 1000, 'easeOutExpo', function () {
                $(this).remove();
                c.data('isAnimate', false);
                a.removeClass('animated').addClass('active');
            });
        });
    }
}
...
var pseudoCheckbox = {
    init: function () {
        var _this = this;
        $('input.pseudo-checkbox-init:visible').each(function () {
            var box = $(this);
            var pseudo = $('<div class="checkbox-pseudo-checkbox"></div>');
            box.removeClass('pseudo-checkbox-init').after(pseudo).css({position: 'absolute', left: '-9999px'});
            box.click(function () {
                if (box.is(':checked')) pseudo.addClass('checkbox-pseudo-checkbox-checked'); else pseudo.removeClass('checkbox-pseudo-checkbox-checked');
            });
            box.focus(function () {
                pseudo.addClass('focused');
            }).blur(function () {
                pseudo.removeClass('focused');
            });
            box.data('pseudo', pseudo);
            _this.setClass(box, pseudo);
        });
    },
    setClass: function (box, pseudo) {
        box = $(box);
        if (!pseudo) pseudo = box.data('pseudo');
        if (box.is(':checked'))pseudo.addClass('checkbox-pseudo-checkbox-checked'); else pseudo.removeClass('checkbox-pseudo-checkbox-checked');
        if (box.is(':disabled')) pseudo.addClass('checkbox-pseudo-checkbox-disabled'); else pseudo.removeClass('checkbox-pseudo-checkbox-disabled');
    }
}
...
var hotBlock = {
    init: function () {
        var b = $('#hot-block');
        $('.float-menu-container a', b).click(function () {
            var a = $(this)
            var c = a.closest('.float-menu-container');
            if (a.hasClass('active') || c.data('isAnimate')) return false;

            c = $('#hot-slideshow-container');
            c.stop();
            var slides = $(a.data('target'));
            var top = -slides.position().top;
            if (parseInt(c.css('margin-top'))) top += parseInt(c.css('margin-top'));
            c.animate({marginTop: top}, 1500, 'easeInOutBack');
        });
        carousel.initAll();
    }
}
...
var carousel = {
    ...
    init: function (o) {
        o = $(o);
        if (o.data('carouselReady')) return false;
        o.data('carouselReady', true);
        var ul = $('ul', o), str = $('.str', o), stl = $('.stl', o), w = $('.carousel-wrapper', o);
        var width = 0;
        $('li', ul).each(function () {
            width += $(this).outerWidth();
        });
        if (width <= w.outerWidth()) {
            str.hide();
            stl.hide();
            w.width(width);
            return false;
        ...
        var newLeft;
        str.click(function () {
            if (o.data('isAnimate')) return false;
            o.data('isAnimate', true);
            var left = parseInt(ul.css('margin-left'));
            if (isNaN(left)) left = 0;
            var next = false;
            $('li', ul).each(function () {
                if (!next && $(this).position().left + $(this).outerWidth() > ww) {
                    next = $(this);
                }
            });
            if (!next || next.size() == 0) {
                var first = $('li:first', ul);
                ul.css('margin-left', (left + first.outerWidth()) + 'px');
                left += first.outerWidth();
                ul.append(first);
                next = first;
            }

            newLeft = left - next.outerWidth();
            ul.animate({marginLeft: newLeft}, 500, function () {
                o.data('isAnimate', false);
            });
        }).mousedown(function () {
            return false;
        });
        ...
        stl.click(function () {
            ...
            if (left == 0) {
                ...
                $('li', ul).each(function () {
                    if (-($(this).position().left) < left) prev = $(this);
                });
            }

            newLeft = left + prev.outerWidth();
            ul.animate({marginLeft: newLeft}, 500, function () {
                o.data('isAnimate', false);
            });
        }).mousedown(function () {
...
var popup = {
    ...
    show: function (e, callback) {
        ...
        this.e = e;
        var _this = this;
        if (!callback) callback = function (p) {
        }
        ...
        var shawl = $('#popup-shawl');
        var isShawl = true;
        if (shawl.size() == 0) {
            shawl = $('<div id="popup-shawl"></div>');
            $('body').append(shawl);
            isShawl = false;
        }
        ...
        shawl.stop().fadeTo(time, 0.4, function () {
            $('body').addClass('lock');
            var w = $(document).width();
            $('body').removeClass('lock');
            w -= $(document).width();
            $('body').addClass('lock').attr('style', 'margin-right:' + w + 'px').append('<div id="popup-wrapper"><div id="popup"></div></div>');
            //$('body').addClass('lock').append('<div id="popup-wrapper"><div id="popup"></div></div>');

            var close = $('<div class="close"></div>');
            var ec = e.clone();

            if ($(document).width() > 768)
                $('#popup').append(ec).width(ec.outerWidth()).hide().fadeTo(time, 1).append(close);//
            else
                $('#popup').append(ec).width($(document).width()).hide().fadeTo(time, 1).append(close);//

            if (ec.data('class-name')) $('#popup').addClass(ec.data('class-name'));
            frm.init();
            close.click(function () {
                _this.close();
            });
            _this.setPos();
            $(window).on('resize.popupPos', function () {
                _this.setPos();
            });
        ...
        $(window).on('keydown.closePopup', function (e) {
    ...
    close: function () {
        $(window).off('keydown.closePopup');
        $(window).off('resize.popupPos');
        if ($(this.e).data('close')) {
            eval($(this.e).data('close'));
            return false;
        }

        /*if (this.checkReturnForm()) {
            this.showReturnForm();
            return false;
        }*/
        ...
        $('#popup-shawl').fadeTo(100, 0, function () {
            $(this).remove();
        });
        if (this.tot) {
            window.clearInterval(this.tot);
    ...
    autoPos: function () {
        var t = 0, i = 0;
        var _this = this;
        if (this.tot) window.clearInterval(this.tot);
    ...
    checkReturnForm: function () {
        var p = $('#popup');
        var form_type = $('input[name="form_type"]', p);
        if (form_type.size()) {
            if (form_type.val() == 'fast') return true;
            if (form_type.val() == 'main' && this.step6) return true;
        }

        return false;
    },
    showReturnForm: function () {
        var p = $('#popup');
        ...
        var resort_name;
        if ($('#popup select[name="resort"]').size()) {
            resort_name = $('#popup select[name="resort"] option[value="' + data.resort + '"]').text();
        } else {
            resort_name = $('select[name="resort"]:first option[value="' + data.resort + '"]').text();
        }


        $('#return-form .text-val-city').text(city_name);
        $('#return-form .text-val-resort').html(country_name + ', ' + resort_name);
        $('#return-form .text-val-date').text(data.date);
        $('#return-form .text-val-days').text(data.nights);

        var cnt = data.adt + ' ' + utils.numberDecline(data.adt, 'взрослых', 'взрослый', 'взрослых');
        ...
        for (var key in data) {
            ...
            if (key == 'form_type')html += '<textarea name="old_form_type">' + data[key] + '</textarea>';
        }
        $('#return-form .hidden-inputs').html(html);

        popup.show('#return-form');
    }
}
...
var frm = {
    init: function () {
        ...
        f.submit(function () {
            ...
            $('*[data-required]', this).each(function () {
                if ($.trim($(this).val()) == '') {
                    $(this).addClass('error');
                    $(this).closest('.item').addClass('item-error');
                    error = true;
                    if (!$(this).data('isClickReady')) {
                        $(this).focus(function () {
                            $(this).removeClass('error');
                            $(this).data('isClickReady', true);
                        });
            ...
            var buttons = $('*[type="submit"]', f);
            buttons.attr('disabled', 'disabled');
            var target = f.data('target');
            if (!target) target = '/ajax/form/';
            $.post(target, f.serialize(), function () {
    ...
    hideInit: function (f) {
        var f = $(f);
        var _this = this;
        $('*[data-next]', f).each(function () {
            _this.checkHide(this, f, true);
        }).change(function () {
            _this.checkHide(this, f);
        });
        $('*[data-variant-next]', f).each(function () {
            _this.checkVariantHide(this, f, true);
        }).change(function () {
            _this.checkVariantHide(this, f)
        })
    },
    checkHide: function (input, f, isFirst) {
        var input = $(input);
        var target = $('*[data-name="' + input.data('next') + '"]', f);
        var val = $.trim(input.val());
        if (isFirst) {
            if (val == '') target.hide(); else target.show();
        } else {
            window.setTimeout(function () {
                var isFocus = false;
                target.each(function () {
                    if (!isFocus) {
                        ...
                        if (input.size() != 0 && !input.hasClass('site-date') && input.is(':visible') && !input.attr('readonly')) {
                            input.focus();
                            isFocus = true;
                        }
                    }
            ...
            if (val == '') target.slideUp(300); else target.slideDown(300, function () {

            });
            popup.autoPos();
        }
    },
    checkVariantHide: function (input, f, isFirst) {
        var input = $(input);
        var target = $('*[data-variant="' + input.data('variant-next') + '"]', f);
        var val = $.trim(input.val());
        if (isFirst) {
            target.hide();
        } else {
            target.slideUp(300);
            popup.autoPos();
            $('*[data-required]', target).removeAttr('data-required');
        }
        var target2 = $('*[data-variant="' + input.data('variant-next') + '"]', f).filter('[data-variant-value="' + val + '"]');
        target2.each(function () {
            if (isFirst) {
                ...
                $(this).slideDown();
                popup.autoPos();
                if ($(this).data('need-required')) $('input', this).attr('data-required', 'required');
            }
        });
    },
    showCommentItem: function (a) {
...
var siteForms = {
    showFast: function (f) {
        if (!f) f = $('#main-top-fast-form');
        var data = $(f).serializeObject();
        popup.show('#fast-form', function (p) {
            var fields = 'departure,resort,date,nights,adt,cnn'.split(',');
            for (var i = 0; i < fields.length; i++) {
                $('input[name="' + fields[i] + '"]', p).val(data[fields[i]]);
            }
        });
    },
    showHot: function (id) {
        popup.show('#hot-form', function (p) {
            $('input[name="hot_id"]', p).val(id);
        });
    },
    getResorts: function (select) {
        var val = $(select).val();
        var f = $(select).closest('form');
        var target = $('select[name="resort"]', f);
        target.html('<option value=""></option>').trigger('chosen:updated').load('/ajax/tur/?action=resorts&country=' + val, function () {
            $(this).trigger('chosen:updated');
        });
    }
}
...
var mainStepForm = {
    goToStep: function (step) {
        if (step == 6) {
            $('.popup-form:visible').addClass('popup-form-step-6');
            $('#popup-wrapper .close').addClass('white');
        } else {
            $('.popup-form:visible').removeClass('popup-form-step-6');
            $('#popup-wrapper .close').removeClass('white');
        }
        switch (step) {
            case 1:
                this._goToStep1();
    ...
    setStep: function (li) {
        ...
        this.goToStep(li.data('step'));
    },
    getLi: function (step) {
        return $('#popup ul.steps li[data-step="' + step + '"]');
    },
    removeAcitveLi: function () {
        $('#popup ul.steps li.active').removeClass('active');
    },

    _goToStep1: function () {
        this.removeAcitveLi();
        this.getLi(1).addClass('active');
        var p = $('#popup');
        $('.step-item', p).hide();
        $('.step-item-1', p).show();
    },
    ...
    _goToStep2: function () {
        var p = $('#popup');
        var val = $('.step-item-1 select option:selected', p).text();
        var li = this.getLi(1);
        li.text(val);
        this.removeAcitveLi();
        $('.step-item', p).hide();
        $('.step-item-2', p).show();
        this.getLi(2).addClass('checked selected active');
        siteDocReady.selectInit();
    },

    _goToStep3: function () {
        var p = $('#popup');
        var val = $('select[name="country"] option:selected', p).text();
        var option = $('select[name="resort"] option:selected', p);
        if (option.size() > 0 && option.val() != 0) val = val;//val=option.text();
        this.getLi(2).text(val);

        this.removeAcitveLi();
        this.getLi(3).addClass('active selected');
        var p = $('#popup');
        $('.step-item', p).hide();
        $('.step-item-3', p).show();
        siteDocReady.datepickerInit();
    },
    _goToStep4: function () {
        var p = $('#popup');
        var val = $.trim($('input[name="date"]', p).val());
        var label = $('.step-item-3 label', p);
        if (val == '') {
            label.addClass('error');
            return false;
        } else {
            label.removeClass('error');
            this.getLi(3).text(val).addClass('checked');
            $('.step-item', p).hide();
            $('.step-item-4', p).show();
            this.removeAcitveLi();
            this.getLi(4).addClass('selected checked active');
        }
    },
    _goToStep5: function () {
        var p = $('#popup');
        var val = parseInt($('input[name="nights"]', p).val());
        this.getLi(4).text(val + ' ' + utils.numberDecline(val, 'дней', 'день', 'дня'));

        this.removeAcitveLi();
        this.getLi(5).addClass('active checked selected');
        var p = $('#popup');
        $('.step-item', p).hide();
        $('.step-item-5', p).show();
    },
    _goToStep6: function () {
        var p = $('#popup');
        var cnt1 = parseInt($('input[name="adt"]', p).val());
        var cnt2 = parseInt($('input[name="cnn"]', p).val());
        var cnt = cnt1 + cnt2;
        this.getLi(5).text(cnt + ' ' + utils.numberDecline(cnt, 'человек', 'человек', 'человека'));

        this.removeAcitveLi();
        this.getLi(6).addClass('active selected');
        var p = $('#popup');
        $('.step-item', p).hide();
        $('.step-item-6', p).show();
        popup.step6 = true;
...
var ajaxPopup = {
    openFast: function (type) {
        this.open('/js/ajax/popup/?type=' + type);
    },
    open: function (url) {
        var shawl = $('#popup-shawl');
        var isShawl = true;
        ...
        shawl.fadeTo(400, 0.4);
        if (!$('#popup-ajax-loader').size())    $('body').append('<div id="popup-ajax-loader"></div>');

        $.get(url, function (data) {
            $('#popup-ajax-loader').remove();
            var div = $('<div />').addClass('popup-form info-popup').append(data);
            popup.show(div);
        });
    }
}
